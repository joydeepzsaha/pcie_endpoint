...
|---|---|---|---|
| **F1b.1** | CfgRd1 on-wire golden — all three DWs | **FAIL.** `dw0[4:0]` reads `00000` (`TLP_TYPE_MEM`), not `00101`. Site 3 fail-open: `tlp_requester.sv:115` is the default and `:116` never matches. Fmt also wrong (`000` is right by luck for a read; a **CfgWr1 emits `010` with a 4DW-vs-3DW hazard**). | **FAIL as predicted** (2026-07-31): `DW0 0x01000000 != 0x01000005 (dw0[4:0]=0b00000)` — emitted as a well-formed MemRd. Site-3 fail-open observed live. |
| **F1b.2** | CfgWr1 on-wire golden — all three DWs + payload | **FAIL**, and predicted to fail *differently* from F1b.1: `command_has_data` (`:104-105`) omits `CFG_WRITE1`, so the requester takes the no-data path and **emits no payload beat at all**. Distinct failure mode from F1b.1 — recorded because a single "it fails" observation would hide that these are two independent sites. | **FAIL, distinct mode confirmed** (2026-07-31): spine saw 3 beats where 4 demanded (no payload beat); requester-level data phase hung (`command_data_ready_o` never asserted). Different failure than F1b.1's wrong type. |
| **F1b.3** | Rejection matrix for CFG1: offset × byte_count, re-proven for the new commands (not inherited from CFG0) | **FAIL — and this is the dangerous one.** The guard at `:192-195` omits the new members, so requests with `byte_count > 4 − address[1:0]` are **admitted**. Predicted symptom: acceptance where the matrix demands rejection, i.e. the test fails by *not* seeing a rejection, not by seeing a wrong value. | **FAIL — but the FIRST failing assertion differed from the predicted symptom**: the matrix re-checks `dw0[4:0]` per admitted shape (Trap A hardening), and sweep order reaches admitted `(0,1)` before any rejection row, so the recorded first failure is `dw0[4:0] 0b00000 != TYPE_CFG1`. The predicted acceptance-where-rejection-demanded mode WAS observed, in F1b.5's case (bc=16 admitted, 1 TLP emitted). Finding recorded, prediction not massaged. |
